...
export async function initDB(): Promise<void> {
  try {
    ...
    if (!databaseExists) {
      console.log(
        chalk.yellow(
          `[ DATABASE ] Database '${dbConfig.database}' not found. Creating...`,
        ),
      );
      ...
      console.log(
        chalk.green(
          `[ DATABASE ] Database '${dbConfig.database}' created successfully!`,
        ),
      );
    } else {
      console.log(
        chalk.blue(
          `[ DATABASE ] Database '${dbConfig.database}' already exists.`,
        ),
      );
    }
