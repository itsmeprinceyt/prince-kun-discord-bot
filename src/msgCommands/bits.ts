...
import { InvalidInput, NoBitsFound, NotTriggeredByYouBits } from "../utility/embeds/karuta-bits";
import { UserBitsSession } from "../types/UserBitsSession.type";
import { Karuta_Bot } from "../utility/utils";
...
export default {
    ...
    async execute(message: Message) {
        ...
        if (!repliedTo) return;
        if (!repliedTo.author.bot || repliedTo.author.id !== Karuta_Bot) return;
...
export async function handleMessageUpdate(oldMessage: Message, newMessage: Message) {
    if (!newMessage.author.bot || newMessage.author.id !== Karuta_Bot) return;
